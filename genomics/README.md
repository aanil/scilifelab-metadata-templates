...
- A [_genomics_technical_metadata.json_](https://github.com/ScilifelabDataCentre/scilifelab-metadata-templates/blob/main/src/scilifelab_metadata_templates/genomics/templates/genomics_technical_metadata.json) listing all required metadata fields with their attribute descriptors and controlled vocabularies fetched from ENA where applicable, for reference. 
- A [ _genomics_technical_metadata.tsv_](https://github.com/ScilifelabDataCentre/scilifelab-metadata-templates/blob/main/src/scilifelab_metadata_templates/genomics/templates/genomics_technical_metadata.tsv) containing only the field names as a header row, to be filled in with the information for individual runs (1 (single or paired) run per row). 
- A [_genomics_template_schema.json_](https://github.com/ScilifelabDataCentre/scilifelab-metadata-templates/blob/main/src/scilifelab_metadata_templates/genomics/templates/genomics_template_schema.json) __schema__ against which a filled-in _.tsv_ can be validated to ensure that it complies with the template. 
